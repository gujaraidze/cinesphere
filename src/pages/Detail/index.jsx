...
import { getMovieDetails, getMovieCredits, getSimilarMovies, IMAGE_ORIGINAL, IMAGE_BASE } from '../../api/tmdb';
...
function Detail() {
  ...
  const fetchCredits  = useCallback(() => getMovieCredits(id),  [id]);
  const fetchSimilar  = useCallback(() => getSimilarMovies(id), [id]);

  const { data: movie,   loading: loadingMovie  } = useFetch(fetchDetails, [id]);
  const { data: credits, loading: loadingCredits } = useFetch(fetchCredits, [id]);
  ...
  if (!movie) return <p style={{ padding: '80px 48px', color: '#999' }}>Movie not found.</p>;
  ...
  return (
    <>
      ...
      <div className="detail-hero">
        {backdropUrl && (
          <img className="detail-hero__img" src={backdropUrl} alt={movie.title} />
        )}
        ...
        <div className="detail-hero__info">
          ...
          <div className="detail-hero__tags">
            {genres.map((g) => (
              <span key={g} className="detail-hero__tag">{g}</span>
            ))}
          </div>
        </div>
      </div>
      ...
      <div className="detail-actions">
        <div className="detail-actions__left">
          <Link to={`/movie/${id}`} className="action-btn action-btn--primary">
            <PlayIcon /> Play Now
          </Link>
          <button className="action-btn action-btn--outline" onClick={() => setTrailerOpen(true)}>
            <VideoIcon /> Watch Trailer
          </button>
          <button className="action-btn action-btn--outline" onClick={handleWatchlist}>
            <BookmarkIcon />
      ...
      <TrailerModal movieId={id} isOpen={trailerOpen} onClose={() => setTrailerOpen(false)} />

      {/* Similar movies row — full bleed */}
